...
describe('calculateProjectMetrics — EVM math', () => {
  ...
  it('CPI ≈ EV / AC', async () => {
    const r = await calculateProjectMetrics(1, 'user_1', 'scrum');
...
describe('calculateProjectMetrics — edge cases', () => {
  it('throws when project is not found', async () => {
    ...
    await expect(calculateProjectMetrics(999, 'user_1', 'scrum')).rejects.toThrow('Project not found');
  });

  it('handles empty task list without throwing', async () => {
    setupMocks([]);
    await expect(calculateProjectMetrics(1, 'user_1', 'scrum')).resolves.toBeDefined();
  });
